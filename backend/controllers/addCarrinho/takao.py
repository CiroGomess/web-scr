...
async def garantir_tela_pesquisa_takao(page) -> None:
    # ✅ seu input real é data-testid="inputSearch" e id="inputSearch"
    selector_busca = "input#inputSearch[data-testid='inputSearch'], input[data-testid='inputSearch'], input#inputSearch"

    try:
        await page.wait_for_selector(selector_busca, state="visible", timeout=12000)
        return
    except Exception:
        pass
...
async def buscar_produto_takao(page, codigo: str) -> None:
    ...
    # digita (angular costuma reagir melhor com type)
    ...
    log_interno("ENTER enviado. Aguardando resultado... (Takao pode ser lento)")

    # Espera aparecer card do resultado (ajuste se seu componente for diferente)
    try:
        await page.wait_for_selector("app-card-produto-home", state="attached", timeout=25000)
    except Exception:
        log_interno("Nenhum card apareceu após a busca (app-card-produto-home).")
        return

    # folga para Angular renderizar botão/preço
